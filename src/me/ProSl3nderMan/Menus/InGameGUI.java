...
public class InGameGUI implements Listener {
	...
	public InGameGUI(Main main) {
		main.getServer().getPluginManager().registerEvents(this, main);
	}
	public static Player IGWatch(Player p) {
		ItemStack ig = new ItemStack(Material.WATCH);
		ItemMeta igMeta = ig.getItemMeta();
		igMeta.setDisplayName("In Game Commands");
		ig.setItemMeta(igMeta);
		p.getInventory().setItem(7, ig);
		p.updateInventory();
		return p;
  }
	
	public static void openIGGUI(Player player) {
		Inventory inv = Bukkit.createInventory(null, Main.MenuConfig.getMenu().getInt("menus.igmenu.slotamount"), ChatColor.DARK_AQUA + "In Game Menu");
		
		for (int i = 0 ; i < Main.MenuConfig.getMenu().getInt("menus.igmenu.slotamount") ; i++) {
		...
		for (String slot : Main.MenuConfig.getMenu().getConfigurationSection("menus.igmenu.slots").getKeys(false)) {
			String direct = "menus.igmenu.slots." + slot;
	...
	public static void OpenGUIListener(InventoryClickEvent event) {
		...
		for (String slot : Main.MenuConfig.getMenu().getConfigurationSection("menus.igmenu.slots").getKeys(false)) {
			if (event.getCurrentItem().getType() == Material.getMaterial(Main.MenuConfig.getMenu().getString("menus.igmenu.slots." + slot + ".block"))) {
				p.closeInventory();
				p.performCommand(Main.MenuConfig.getMenu().getString("menus.igmenu.slots." + slot + ".command"));
				return;
			}
		}
	}
